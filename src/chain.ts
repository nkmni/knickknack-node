...
class ChainManager {
  ...
  async init() {
    ...
    try {
      ...
      logger.debug(
        `Retrieved cached longest chain tip ${tip.blockid} at height ${height}.`,
      );
    } catch {
      tip = await Block.makeGenesis();
      height = 0;
      logger.debug(
        `No cached longest chain exists. Initializing to genesis ${tip.blockid} at height ${height}.`,
      );
      inited = true;
    }
  ...
  async save() {
    await db.put('longestchain', [
      this.longestChainTip,
      this.longestChainHeight,
    ]);
  }
  async onValidBlockArrival(block: Block) {
    ...
    if (height > this.longestChainHeight) {
      ...
      const [lca, shortFork, longFork] = await Chain.getForks(
        this.longestChainTip,
        block,
      );
      if (shortFork.blocks.length !== 0) {
        logger.info(
          `Reorged chain by abandoning a temporary fork of ` +
...
export class Chain {
  ...
  static async getForks(b1: Block, b2: Block): Promise<[Block, Chain, Chain]> {
    if (!b1.valid) {
      throw new Error(
        `Attempted to compare forks of blocks ${b1.blockid} and ${b2.blockid}, but ${b1.blockid} is invalid.`,
      );
    }
    if (!b2.valid) {
      throw new Error(
        `Attempted to compare forks of blocks ${b1.blockid} and ${b2.blockid}, but ${b2.blockid} is invalid.`,
      );
    }
    ...
    if (l1 === null || l2 === null) {
      throw new Error(
        'Attempting to get forks between chains with no known length',
      );
    }
    const b2Parent = await b2.loadParent();
    if (b2Parent === null) {
      throw new Error(
        'Attempting to get forks between chains with no shared ancestor',
      );
    }
    if (l1 === l2) {
