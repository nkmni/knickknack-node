  ...
  async resolve(): Promise<Output> {
    ...
    if (this.index >= refTx.outputs.length) {
      throw new AnnotatedError(
        'INVALID_TX_OUTPOINT',
        `Invalid index reference ${this.index} for transaction ${this.txid}. The transaction only has ${refTx.outputs.length} outputs.`,
      );
    }
  ...
  async validate(idx?: number, block?: Block) {
    ...
    if (this.isCoinbase()) {
      if (this.outputs.length > 1) {
        throw new AnnotatedError(
          'INVALID_FORMAT',
          `Invalid coinbase transaction ${this.txid}. Coinbase must have only a single output.`,
        );
      }
      if (block !== undefined && idx !== undefined) {
        ...
        if (idx > 0) {
          throw new AnnotatedError(
            ...
            `Coinbase transaction ${this.txid} must be the first in block.`,
          );
        }
      }
      this.fees = 0;
    ...
    const inputValues = await Promise.all(
      this.inputs.map(async (input, i) => {
        if (
          blockCoinbase !== undefined &&
          input.outpoint.txid === blockCoinbase.txid
        ) {
          throw new AnnotatedError(
            'INVALID_TX_OUTPOINT',
            `Transaction ${this.txid} is spending immature coinbase`,
          );
        }
        ...
        if (input.sig === null) {
          throw new AnnotatedError(
            ...
            `No signature available for input ${i} of transaction ${this.txid}`,
          );
        }
        if (!(await ver(input.sig, unsignedTxStr, prevOutput.pubkey))) {
          throw new AnnotatedError(
            'INVALID_TX_SIGNATURE',
            `Signature validation failed for input ${i} of transaction ${this.txid}`,
          );
        }
    ...
    if (sumInputs < sumOutputs) {
      throw new AnnotatedError(
        ...
        `Transaction ${this.txid} does not respect the Law of Conservation. Inputs summed to ${sumInputs}, while outputs summed to ${sumOutputs}.`,
      );
    }
