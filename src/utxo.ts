...
export class UTXOSet {
  ...
  async apply(tx: Transaction, idx?: number, block?: Block) {
    ...
    logger.debug(
      `Checking ${tx.inputs.length} inputs of transaction ${tx.txid} against the UTXO set.`,
    );
    for (const input of tx.inputs) {
      logger.debug(
        `Checking input ${input.outpoint} of transaction ${tx.txid} against the UTXO set.`,
      );
      const outpointStr: string = input.outpoint.toString();

      logger.debug(
        `Checking to see if outpoint ${outpointStr} is unspent in UTXO outpoints ${this.outpoints}.`,
      );
      if (!this.outpoints.has(outpointStr)) {
        logger.debug(
          `Transaction ${tx.txid} consumes ${outpointStr} which is not in the UTXO set.`,
        );
        throw new AnnotatedError(
          'INVALID_TX_OUTPOINT',
          `Transaction consumes output (${JSON.stringify(
            outpointStr,
      ...
      logger.debug(
        `Checking if outpoint ${outpointStr} is being respent in the same tx.`,
      );
      if (seen.has(outpointStr)) {
        logger.debug(
          `Transaction ${tx.txid} has two different inputs spending the same outpoint ${outpointStr}`,
        );
        throw new AnnotatedError(
          'INVALID_TX_OUTPOINT',
          'Two different inputs of the same transaction are spending the same outpoint',
        );
      }
      logger.debug(
        `Outpoint ${outpointStr} has not been spent in the same tx.`,
      );
