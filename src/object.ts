...
class ObjectManager {
      ...
      async (obj: TransactionObjectType) => {
        ...
        if (!tx.isCoinbase()) {
          ...
          if (!success) {
            throw new AnnotatedError(
              'INVALID_TX_OUTPOINT',
              'Failed to add invalid transaction to mempool',
            );
          }
        }
        return tx;
  ...
  async retrieve(objectid: ObjectId, peer: Peer): Promise<ObjectType> {
    ...
    logger.debug(
      `Object ${objectid} not in database. Requesting it from peer ${peer.peerAddr}.`,
    );
    ...
    logger.debug(
      `Object ${objectid} was retrieved from peer ${peer.peerAddr}.`,
    );
