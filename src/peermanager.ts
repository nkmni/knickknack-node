...
class PeerManager {
  ...
  peerDiscovered(peerAddr: string) {
    ...
    if (peerParts.length !== 2) {
      logger.warn(
        `Remote party reported knowledge of invalid peer ${peerAddr}, which is not in the host:port format; skipping`,
      );
      return;
    }
    ...
    if (!(port >= 0 && port <= 65535)) {
      logger.warn(
        `Remote party reported knowledge of peer ${peerAddr} with invalid port number ${port}`,
      );
      return;
    }
    if (!isValidHostname(host)) {
      logger.warn(
        `Remote party reported knowledge of invalid peer ${peerAddr}; skipping`,
      );
      return;
    }
